...
serve(async (req) => {
  ...
  if (!requestId) return json(400, { ok: false, error: "ACTIVATION_REQUEST_ID_REQUIRED" });
  if (!decision) return json(400, { ok: false, error: "DECISION_REQUIRED" });
  ...
  if (reqRes.error || !reqRes.data?.id) return json(404, { ok: false, error: "REQUEST_NOT_FOUND" });
  ...
  const reviewReq = await serviceClient
    ...
    .single();
  if (reviewReq.error) return json(500, { ok: false, error: "REQUEST_REVIEW_UPDATE_FAILED" });
