...
export type EmployeeHourRow = WorkingHourRow & {
...
export function validateBooking(input: {
  ...
  const {employeeId, start, end, bookings = [], timeOff = [], salonHours = [], employeeHours = [], excludeBookingId} = input;

  if (!employeeId) {
    return {ok: false, reason: 'select_employee'};
  }
  ...
  if (window.breakStart && window.breakEnd) {
    ...
    if (overlapsBreak) {
      return {ok: false, reason: 'inside_break'};
    }
  }
