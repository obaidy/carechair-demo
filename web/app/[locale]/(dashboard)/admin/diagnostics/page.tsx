...
export default async function AdminDiagnosticsPage({params, searchParams}: Props) {
  const {locale} = await params;
  const query = await searchParams;
  const session = await readAuthSession();

  if (!session || session.role !== 'superadmin') {
    redirect(`/${locale}/login?next=/${locale}/admin/diagnostics`);
  }

  const basePath = `/${locale}/admin/diagnostics`;
  ...
  async function createTestSalonAction() {
    ...
    if (!current || current.role !== 'superadmin' || !current.userId) {
    ...
    const slug = `diag-admin-${stamp.toString(36)}`.slice(0, 60);
    const name = `Admin Diagnostics ${new Date(stamp).toISOString().slice(11, 19)}`;
  ...
  const salonsRes = await supabase
    .from('salons')
    .select('id,name,slug,status,city,area,is_public,is_active,created_by,updated_at,created_at')
    .order('updated_at', {ascending: false})
    .limit(10);

  const activationRes = await supabase
    .from('activation_requests')
    .select('id,salon_id,status,requested_by,created_at,reviewed_at,admin_notes,submitted_data')
  ...
  const summary = {
    ...
    membershipsCount: (membershipsRes.data || []).length,
  ...
  return (
    <div className="cc-section">
      <section className="panel hero-lite">
        <h1>Super Admin Diagnostics</h1>
        <p className="muted">Server/client visibility for status, queue, and membership consistency.</p>
      </section>
      ...
      <section className="panel" style={{marginTop: 12}}>
        ...
        <pre style={{whiteSpace: 'pre-wrap', fontSize: 12, lineHeight: 1.4}}>{JSON.stringify(membershipsRes.data || [], null, 2)}</pre>
      </section>
      ...
      <section className="panel" style={{marginTop: 12}}>
        <h3>Activation Requests (last 10)</h3>
