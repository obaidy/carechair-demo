...
import {useSalonAdminSession} from '@/components/dashboard/SalonAdminSessionContext';
...
export default function SalonAdminPage() {
  ...
  const params = useParams<{slug?: string | string[]; module?: string[] | string}>();
  const router = useRouter();
  const tx = useTx();
  const slugValue = params?.slug;
  const slug = decodeURIComponent(String(Array.isArray(slugValue) ? slugValue[0] : slugValue || ''));
  const moduleValue = params?.module;
  ...
  const {unlocked, setUnlocked} = useSalonAdminSession();
  ...
  const salonAccess = useMemo(() => deriveSalonAccess(salon, new Date(), tr), [salon, tr]);
  const writeLocked = unlocked && !salonAccess.canManage;
  const setupPaymentLink = String(
    process.env.NEXT_PUBLIC_SETUP_PAYMENT_LINK || process.env.NEXT_PUBLIC_VITE_SETUP_PAYMENT_LINK || ""
  ).trim();
  const trialRemaining = getTrialRemainingLabel(salon?.trial_end_at || salon?.trial_end, new Date(), tr);
    ...
    >
      {!unlocked ? (
        <Card>
          ...
          >
            <TextInput label={t("admin.passcodeLabel")} type="password" value={adminPass} onChange={(e) => setAdminPass(e.target.value)} />
                ...
                <Card>
                  ...
                  <div className="billing-stats-grid">
                    ...
                    <div className="billing-stat-card">
                      ...
                      <b>{getBillingStatusLabel(salon.status || salon.subscription_status || salon.billing_status, tr)}</b>
                    </div>
                    <div className="billing-stat-card">
                      <span>{tr("admin.billing.subscriptionEnds", "Subscription ends")}</span>
                      <b>{formatBillingDate(salon.current_period_end, i18n.language || "en-US")}</b>
                    </div>
                    <div className="billing-stat-card">
                      ...
                      <b>
                        {salon.trial_end_at || salon.trial_end
                          ? `${trialRemaining || tr("admin.billing.running", "Running")} (${tr(
                              "admin.billing.ends",
                              "Ends {{date}}",
                              { date: formatBillingDate(salon.trial_end_at || salon.trial_end, i18n.language || "en-US") }
                            )})`
