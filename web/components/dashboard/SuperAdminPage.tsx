...
export default function SuperAdminPage() {
  ...
  const detailSalonId = String(routeParams?.id || "");
  ...
  const i18n = useMemo(
    () => ({language: locale, dir: () => (String(locale || "en").startsWith("ar") ? "rtl" : "ltr")}),
    [locale]
  );
  const t = useCallback(
  ...
  const isOverviewPage = routePath.startsWith("/sa/overview");
  const isApprovalsPage = routePath.startsWith("/sa/approvals");
  ...
  function startEditSalon(row) {
    ...
    setSalonEditDraft({
      ...
      slug: normalizeSlug(row.slug || row.name || ""),
  ...
  async function saveSalonInfo(row) {
    ...
    const slug = normalizeSlug(salonEditDraft.slug || salonEditDraft.name);
        ...
        <section className="panel">
          ...
          >
            ...
            <label className="field">
              ...
              <input className="input" type="password" value={codeInput} onChange={(e) => setCodeInput(e.target.value)} />
            </label>
        ...
        <>
          ...
          <section className="panel superadmin-overview-panel">
            ...
            <div className="row-actions" style={{ marginTop: 12 }}>
              ...
              <Button type="button" variant="ghost" onClick={() => navigate("/sa/approvals")}>
                {t("superadmin.approvals")}
              </Button>
          ...
          <section className="panel superadmin-create-panel">
            ...
            <form className="grid two" onSubmit={createSalon}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm((p) => ({
                      ...p,
                      name: e.target.value,
                      slug: p.slug ? p.slug : normalizeSlug(e.target.value),
                    }))
                  }
                />
              </label>
              ...
              <label className="field">
                ...
                <input className="input" value={form.slug} onChange={(e) => setForm((p) => ({ ...p, slug: normalizeSlug(e.target.value) }))} />
              </label>
              ...
              <label className="field">
                ...
                <input className="input" type="password" value={form.admin_passcode} onChange={(e) => setForm((p) => ({ ...p, admin_passcode: e.target.value }))} />
              </label>
                        ...
                        {editingSalonId === row.id ? (
                          ...
                          >
                            ...
                            <TextInput
                              ...
                              onChange={(e) => setSalonEditDraft((p) => ({ ...p, slug: normalizeSlug(e.target.value) }))}
                            />
                            <TextInput
                    ...
                    return (
                      <div className="settings-row superadmin-salon-row" key={`approval-${row.id}`}>
                        ...
                        <div className="row-actions superadmin-salon-actions">
                          ...
                          <Button variant="ghost" onClick={() => navigate(`/sa/${row.id}`)}>
                            {t("superadmin.view")}
                          </Button>
                          <Button
                            variant="danger"
                    ...
                    <div className="settings-row">
                      ...
                      <div style={{ width: "100%" }}>
                        ...
                        >
                          ...
                          <TextInput
                            label={t("superadmin.form.slug")}
                            value={editingSalonId === selectedSalon.id ? salonEditDraft.slug : normalizeSlug(selectedSalon.slug || selectedSalon.name || "")}
                            onChange={(e) => {
                              if (editingSalonId !== selectedSalon.id) startEditSalon(selectedSalon);
                              setSalonEditDraft((p) => ({ ...p, slug: normalizeSlug(e.target.value) }));
                            }}
                          />
                          <TextInput
                            label={t("superadmin.form.area")}
